...
def test_urgent_note_classified_correctly(note):
    """A note with URGENT must be classified as URGENT."""
    text = "URGENT: Patient experiencing severe chest pain, call doctor immediately."
    result = classify(text)
    assert result["label"] == "urgent", f"Expected 'urgent', actual: {result}"

def test_classification_score_range(note):
    """Confidence Score must be betweeen 0 and 1."""
    result = classify(note["text"])
